...
insert into ogrenciler20 values(120, 'Ali Can', 'Ankara', 75);
insert into ogrenciler20 values(121, 'Veli Mert', 'Trabzon', 85);
insert into ogrenciler20 values(122, 'Ayşe Tan', 'Bursa', 65);
insert into ogrenciler20 values(123, 'Derya Soylu', 'Istanbul', 95);
insert into ogrenciler20 values(124, 'Yavuz Bal', 'Ankara', 85);
insert into ogrenciler20 values(125, 'Emre Gül', 'Hatay', 90);
insert into ogrenciler20 values(126, 'Harun Reşit', 'Isparta', 100);
select * from ogrenciler20;

--Soru: ogrenciler tablosundaki id ve isim sutununu getiriniz

select id,isim from ogrenciler20;

--SORU: Sınav notu 80'den büyük olan öğrencilerin tüm bilgilerini listele

select * from ogrenciler20 where sinav_notu>80;

--SORU: Adresi Ankara olan ögrencilerin tüm bilgilerini listele

select * from ogrenciler20 where adres='Ankara';

 
-SORU: Sınav notu 85 ve adresi Ankara olan öğrenci ismini listele

select isim from ogrenciler20 where sinav_notu = 85 and adres='Ankara';

--SORU: .Sınav notu 65 veya 85 olan ogrencilerin tüm bilgilerini listele

select * from ogrenciler20 where sinav_notu=65 or sinav_notu=85;

select * from ogrenciler20 where sinav_notu in (65,85);

--SORU: .Sınav notu 65 ve 85 arasında olan ogrencilerin tüm bilgilerini listele

select * from ogrenciler20 where sinav_notu between 65 and 85;

--SORU: .id'si 122 ve 125 arasında olmayan ögrencilerin isim ve sınav notu listele

select isim,sinav_notu from ogrenciler20 where id not between 122 and 125;


--SORU: sinav_notu 75 olan satırı siliniz
delete from ogrenciler20 where sinav_notu=75;

--SORU: adres'i Trabzon olan satırı silin
delete from ogrenciler20 where adres='Trabzon';

--SORU: ismi Derya Soylu veya Yavuz Bal olan satırları siliniz
delete from ogrenciler20 where isim='Derya Soylu' or isim='Yavuz Bal';

--SORU: sınav notu 100 den küçük olan satırları siliniz
delete from ogrenciler20 where sinav_notu<100;

--Tablodaki tum satirlari siliniz
delete from ogrenciler20;
